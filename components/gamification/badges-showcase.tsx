...
const getBadgeProgress = (
  ...
  try {
    ...
    let current = 0;
    const target = criteria.count || criteria.days || 0;
...
export default function BadgesShowcase({ userId }: BadgesShowcaseProps) {
  ...
  const groupedBadges = categories.reduce((acc: Record<string, Badge[]>, category) => {
    acc[category] = badges.filter((b) => b.category === category);
    return acc;
  }, {});

  // Show message if no badges exist
  if (badges.length === 0) {
    return (
      <Card>
        <CardHeader>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {categoryBadges.map((badge) => {
                  ...
                  const progress = !isEarned
                    ? getBadgeProgress(badge, profile)
                    : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="text-white text-center text-xs font-medium line-clamp-2">
                          {badge.name}
                        </p>
                        {badge.points > 0 && (
                        ...
                        {/* Progress indicator for locked badges - always visible */}
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                        ...
                        {/* Rarity badge */}
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium uppercase ${RARITY_BG_COLORS[rarity]}`}
                        >
                          {badge.rarity}
                        </span>
